...
namespace JonathonOH.RoadGeneration.ConvexShape2D.Tests.ConvexHullTests.TransformationTests
{
	internal class TransformationConsistencyTests
	{
		...
		private static void RunTransformThenInverseTest(ConvexHull hull, Pose pose)
		{
			ConvexHull transformedHull = hull.TransformBy(pose);
			ConvexHull result = transformedHull.InverseTransformBy(pose);
			AssertHullVerticesEqual(hull, result);
		}

		private static void RunInverseThenTransformTest(ConvexHull hull, Pose pose)
		{
			ConvexHull transformedHull = hull.InverseTransformBy(pose);
			ConvexHull result = transformedHull.TransformBy(pose);
			AssertHullVerticesEqual(hull, result);
		}
